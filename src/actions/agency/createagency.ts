...
export const createAgency = async (args: createAgencyPayload): Promise<ApiResponseType<agency | null>> => {
    try {
        ...
        if (!user) return { status: false, data: null, message: "Invalid user id.Try Again", apiurl: "createAgency" };

        const agencyuser = await prisma.agency.findFirst({ where: { email: args.email } });
        if (agencyuser) return { status: false, data: null, message: "Email already exist.Try Again", apiurl: "createAgency" };
        ...
        if (!updateduser) return { status: false, data: null, message: "Something want wrong unable to update user.", apiurl: "createAgency" };
...
export const uploadagencyLogo = async (args: updateagencyPayload): Promise<ApiResponseType<string | null>> => {
    try {
        ...
        return { status: false, data: null, message: errorToString(e), apiurl: "uploadagencyLogo", };
    }
}

export const uploadagencyBanner = async (args: updateagencyPayload): Promise<ApiResponseType<string | null>> => {
    try {
        const file_name = `${new Date().getTime()}_banner.${args.name.split(".").pop()}`;
        const buffer = Buffer.from(args.arrayBuffer, "base64");
        ...
        return { status: false, data: null, message: errorToString(e), apiurl: "uploadagencyBanner", };
    }
}